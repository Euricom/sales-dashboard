...
const CollapsibleCardGroup: React.FC<CollapsibleCardGroupProps> = ({
  ...
  return (
      ...
      >
        <CollapsibleTrigger
          className={`h-[4.75rem] p-0 text-primary hover:bg-primary/10 hover:text-accent-foreground rounded-xl ${isOpen ? "mr-1" : "mr-0"} relative`}
        >
          <h4 className="text-sm font-semibold -rotate-90">{label}</h4>
          {!isOpen && <div className="absolute text-gray-600 top-[0px] right-[1px] text-[10px]">{data.length}</div>}
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="flex h-[4.75rem] justify-center bg-primary px-3.5 py-2 rounded-2xl">
